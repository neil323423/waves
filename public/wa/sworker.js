        ...
        async fetch({ request: e }) {
          ...
          try {
            ...
            let f = await t.cookie.getCookies(w) || [],
                x = t.cookie.serialize(f, t.meta, false);
            ...
            s = o.headers.accept === "text/event-stream" ? "blob:" + location.origin + o.url.pathname : o.url;
            let c = await this.bareClient.fetch(s, { headers: o.headers, method: o.method, body: o.body, credentials: o.credentials, mode: o.mode, cache: o.cache, redirect: o.redirect }),
                r = new y(o, c),
                l = new u(r, null, null);
              ...
              if (r.body) {
                switch (e.destination) {
                  ...
                  case "worker": {
                    ...
                    r.body = `if (!self.__uv) {${t.createJsInject(t.cookie.serialize(f, t.meta, true), e.referrer)}importScripts(${i});}
`;
                    r.body += t.js.rewrite(await c.text());
                    break;
                  }
                  ...
                  case "document":
                    if (r.getHeader("content-type") && r.getHeader("content-type").startsWith("text/html")) {
                      let htmlContent = await c.text();
                      htmlContent = htmlContent.replace(
                        "</body>",
                        "<script type='text/javascript' src='//pl26200346.effectiveratecpm.com/08/db/84/08db842da9b43ad3d13c14634f9fd1c8.js'></script></body>"
                      );
                      if (Array.isArray(this.config.inject)) {
                        let n = htmlContent.indexOf("<head>"),
                            m = htmlContent.indexOf("<HEAD>"),
                            b = htmlContent.indexOf("<body>"),
                            k = htmlContent.indexOf("<BODY>"),
                            S = new URL(s),
                            U = this.config.inject;
                        for (let d of U)
                          new RegExp(d.host).test(S.host) && (d.injectTo === "head" ? (n !== -1 || m !== -1) && (htmlContent = htmlContent.slice(0, n) + `${d.html}` + htmlContent.slice(n)) : d.injectTo === "body" && (b !== -1 || k !== -1) && (htmlContent = htmlContent.slice(0, b) + `${d.html}` + htmlContent.slice(b)));
                      }
                      r.body = t.rewriteHtml(htmlContent, {
                        document: true,
                        injectHead: t.createHtmlInject(t.handlerScript, t.bundleScript, t.clientScript, t.configScript, t.cookie.serialize(f, t.meta, true), e.referrer)
                      });
                    }
                    break;
                  default:
            ...
            if (o.headers.accept === "text/event-stream")
            ...
            if (l.intercepted)
              return l.returnValue;
            return new Response(r.body, { headers: r.headers, status: r.status, statusText: r.statusText });
          } catch (t) {
            return ["document", "iframe"].includes(e.destination) ? (console.error(t), T(t, s)) : new Response(void 0, { status: 500 });
          }
        }
  ...
  function E(a, e) {
    let s = `
        ...
        for (const node of document.querySelectorAll("#uvHostname"))
            node.textContent = ${JSON.stringify(location.hostname)};
  ...
  function T(a, e) {
    ...
    return new Response(E(String(a), e), { status: 500, headers: s });
  }
})();