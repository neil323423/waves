  ...
  async function registerSW() {
    try {
      if (!navigator.serviceWorker) {
        console.log("%c[⚠️]%c Service Workers are not supported by this browser.", "color: orange; font-weight: bold;", "color: inherit;");
        return;
      }

      console.log("%c[🌐]%c Initializing connection to WebSocket server at: " + wispUrl, "color: purple; font-weight: bold;", "color: inherit;");

      const savedTransport = localStorage.getItem('transport') || "epoxy";
      switchTransport(savedTransport);
      updateTransportUI(savedTransport);

      console.log(`%c[🚀]%c Using ${savedTransport} transport.`, "color: purple; font-weight: bold;", "color: inherit;");

      console.log("%c[⚙️]%c Registering Service Worker...", "color: purple; font-weight: bold;", "color: inherit;");
      await navigator.serviceWorker.register("/sw.js", { scope: '/$/' });
      console.log("%c[✅]%c Service Worker registered successfully.", "color: purple; font-weight: bold;", "color: inherit;");
